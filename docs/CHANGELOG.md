...
- **Daily disk housekeeping.** Nothing in the tree rotated: the production host had grown ~72 MB of never-truncated JSONL and 377 MB of LangGraph checkpoints, none of it any job's responsibility. Rotation is **copy-truncate, never rename** — launchd and the file handlers hold the inode for the life of the process, so a rename leaves every writer bound to the old file. A checkpoint is pruned only when its age is **known** (LangGraph's UUID6 ids carry a 60-bit timestamp) and only **a whole thread at a time**, so a surviving conversation never loses part of its lineage; an id that will not parse as a sane date is kept rather than deleted on a guess.
...
- **Profile readiness.** Every input a shipped engine depends on that only *you* can state, in one surface (`GET /api/profile_readiness`, rendered as marks and a switchboard on `/context`): whether it is on file, and **which shipped feature is inert while it is not**. Five times a slice had shipped complete and correct and then sat dark because the store behind it was empty — the drawdown playbook, the drift band, the IPS caps, the few-shot pool, the wealth goal. Every one of those times the app was right to refuse to author the value, and every one of those times the cost landed as an invisible blank. This reports emptiness and names the consequence; it never authors, defaults, suggests, exemplifies or ranges a value — not as a placeholder, not as an "e.g.", not as a "most people" — and a test scans every string it emits to keep it that way. It is deliberately not a nag: a blank is a valid answer, nothing is scored or chased, and the switchboard counts **features switched off**, not sentences of prose.
...
- **Ledger statistics counted rows, not calls.** A restated recommendation writes several rows, so the partial-hold stats were reporting a sample more than twice its true size (9 graded rows were 4 distinct calls). Statistics now group by `(ticker, action, supersession event)`. Separately, a call that was superseded before its horizon elapsed used to retire unscored; it is now graded **at supersession**, which recovered stranded legs the ledger had silently dropped.
...
- **Fixtures are synthetic, always.** A fixture derived from a production incident gets rewritten with invented data before it is committed. This rule needs stating because every secret-scanning hook in the pipeline passes such a file: holdings are not secrets, so nothing automated will stop them.
- Suite grew to ~1,675 tests across 135 files alongside the advisor work.
